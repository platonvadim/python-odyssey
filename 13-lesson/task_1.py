...
class GoogleProduct(AppleProduct):
    def combine_products(self, product):
        if type(product) != GoogleProduct:
            return "Produsul nu poate fi conectat"
        if product.culoare == self.culoare:
            return "Produsul nu poate fi conectat deoarece culorile coincid"
        self.produs_conectat = product
        return "Produsul a fost conectat cu succes"
# CODUL TĂU VINE MAI SUS:
...
# CODUL TĂU VINE MAI JOS:
class Magazin:
    def vinde_produs(self, produs: Produs):
        return f"Produsul {produs.numele} a fost vândut cu succes"

    def returneaza_produs(self, produs: Produs):
